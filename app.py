...
st.set_page_config(page_title="Sirphire Camera Lens Audit", layout="wide")
...
with st.sidebar:
    ...
    category_url = st.text_input(
        "Category URL",
        value="https://in.sirphire.com/google-pixel-10a-5g-back-covers",
    )

    st.header("Speed")
    max_products = st.number_input("Max products collect from category", min_value=10, max_value=10000, value=1000, step=100)
    batch_size = st.number_input("Products per resume click", min_value=1, max_value=500, value=10, step=10)
    workers = st.number_input("Parallel workers", min_value=1, max_value=5, value=1, step=1)

st.warning("Safe mode: product checking ke liye Parallel workers = 1 rakho. Category collect browser se JS load karega.")
    ...
    if st.button("▶️ Resume / Process next batch", type="primary", use_container_width=True):
        try:
            ...
            if not claimed:
                ...
                st.info(f"{len(claimed)} products claim hue. Checking start...")
...
try:
    ...
    if records:
        st.dataframe(pd.DataFrame(records), use_container_width=True, height=500)
    else:
